...
// --- Data for Models (copied from VirtualModels component) ---
const femaleModels = [
...
const AdCreator: React.FC = () => {
    ...
    const { selectedTemplateId, assetFile, generatedVideos } = state.adCreatorState;
    const [isProcessing, setIsProcessing] = useState(false);
    const [adType, setAdType] = useState<'articles' | 'models'>('articles');

    // State for model selection
    const [selectedGender, setSelectedGender] = useState<'female' | 'male'>('female');
    const modelsToShow = useMemo(() => adType === 'models' && selectedGender === 'female' ? femaleModels : maleModels, [adType, selectedGender]);
    ...
    const handleVideoHover = (e: React.MouseEvent<HTMLVideoElement>, action: 'play' | 'pause') => {
        if (action === 'play') {
            e.currentTarget.play().catch(error => console.log("Autoplay was prevented.", error));
        } else {
    ...
    const handleGenerateVideo = () => {
        ...
        setTimeout(() => {
            ...
            const videoUrl = template.videoUrl; // Use the template's own video as the result
                 ...
                 <div className="flex flex-col flex-1 gap-6 min-h-0">
                    <section className="flex flex-col gap-4">
                        ...
                        <div className="flex gap-4">
                            <button onClick={() => setAdType('articles')} className={`flex items-center gap-3 px-4 py-3 rounded-lg border-2 w-1/2 justify-center transition-colors ${adType === 'articles' ? 'border-primary text-primary dark:text-text-primary-dark bg-primary/10' : 'border-slate-200 dark:border-white/20 text-text-secondary-light dark:text-text-secondary-dark hover:bg-slate-100 dark:hover:bg-white/10'}`}>
                                <span className="material-symbols-outlined">inventory_2</span>
                                <span>Artículos</span>
                            </button>
                            <button onClick={() => setAdType('models')} className={`flex items-center gap-3 px-4 py-3 rounded-lg border-2 w-1/2 justify-center transition-colors ${adType === 'models' ? 'border-primary text-primary dark:text-text-primary-dark bg-primary/10' : 'border-slate-200 dark:border-white/20 text-text-secondary-light dark:text-text-secondary-dark hover:bg-slate-100 dark:hover:bg-white/10'}`}>
                           ...
                           {adType === 'models' && (
                                <section className="flex flex-col gap-3">
                                    <div className="flex items-center justify-between">
                                        <h2 className="text-text-primary-light dark:text-white text-lg font-bold leading-tight tracking-[-0.015em]">Sección 2: Elige un Modelo</h2>
                                        <div className="flex bg-slate-100 dark:bg-black/20 p-1 rounded-lg">
                                            <button onClick={() => setSelectedGender('female')} className={`py-1 px-3 text-sm font-bold rounded-md transition-colors ${selectedGender === 'female' ? 'bg-primary text-white shadow' : 'text-text-secondary-light dark:text-text-secondary-dark'}`}>Mujer</button>
                                            <button onClick={() => setSelectedGender('male')} className={`py-1 px-3 text-sm font-bold rounded-md transition-colors ${selectedGender === 'male' ? 'bg-primary text-white shadow' : 'text-text-secondary-light dark:text-text-secondary-dark'}`}>Hombre</button>
                                        </div>
                                    </div>
                                        ...
                                        <div className="flex overflow-x-auto custom-scrollbar -mx-2 px-2 py-4">
                                            <div className="flex items-stretch gap-4">
                                                {modelsToShow.map(model => (
                                                    <div key={model.id} onClick={() => setSelectedModel(model.url)} className={`flex-shrink-0 w-32 aspect-[3/4] rounded-xl overflow-hidden cursor-pointer ring-2 transition-all ${selectedModel === model.url ? 'ring-primary ring-offset-2 ring-offset-background-light dark:ring-offset-background-dark' : 'ring-transparent hover:ring-primary/50'}`}>
                            ...
                            <section className="flex flex-col gap-4 bg-surface-light dark:bg-surface-dark p-6 rounded-xl flex-1">
                                ...
                                <div className="flex items-center justify-center w-full flex-1 rounded-xl bg-background-light dark:bg-background-dark min-h-[250px]">
                                    {isProcessing ? (
                                        ...
                                        <video src={generatedVideos.a} controls autoPlay loop className="w-full h-full object-contain rounded-lg" />
                                    ) : (
